...
### 라우팅 키
`InMemoryRouteConfigReader`는 routes를 **source type**(e.g. `"BESZEL"`)으로 인덱싱한다. `Update()`에서 `sourceID → sourceType` 변환을 수행하므로, `GetChannels`는 반드시 `string(alert.Source)`(타입 값)를 넘겨야 한다.
